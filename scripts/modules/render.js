import {calculateDewPoint, convertPressure, getCurrentDateTime, getWeatherForecastData} from "./helpers.js";

export const renderWidgetToday = (widget, data) => {

    const {minutes, year, month, dayOfMonth, hours, dayOfWeek} = getCurrentDateTime();
    ...
    widget.insertAdjacentHTML(
        ...
        <div class="widget__wheather">
            ...
            <p class="widget__temp-big">${(data.main.temp - 273.15).toFixed(1)}</p>
...
export const renderWidgetForecast = (widget, data) => {
    ...
    const items = forecastData.map((item) => {
        ...
        widgetDayItem.insertAdjacentHTML('beforeend', `
          ...
          ${(item.minTemp - 273.15).toFixed(1)}°/${(item.maxTemp - 273.15).toFixed(1)}°
            </p>
        `)
